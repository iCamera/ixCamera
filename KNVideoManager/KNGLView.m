...
- (BOOL)loadShaders {
    ...
    glBindAttribLocation(program_, 1, "inputTextureCoordinate");
    ...
    //
    videoFrameUniform = glGetUniformLocation(program_, "videoFrame");
...
- (void)updateVertices {
    ...
    float w             = (height * dd / (float)backingHeight_);
    float h             = (width  * dd / (float)backingWidth_ );
...
- (void)renderBuffer:(CVImageBufferRef)pixelBuffer {
    ...
    frameSize_ = CGSizeMake(bufferWidth, bufferHeight);
    [self updateVertices];
